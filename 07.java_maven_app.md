...
4. 🧪 `Ejecución de Pruebas Unitarias con Maven en Jenkins`. Durante el build, Maven ejecutará las pruebas definidas con
...
## 🧪 4. Ejecución de Pruebas Unitarias con Maven en Jenkins

En esta sección validaremos que nuestra aplicación Java funcione correctamente ejecutando sus `pruebas unitarias`
desde `Jenkins`.
...
Sin embargo, cuando trabajamos con el paso nativo `“Ejecutar tareas 'maven' de nivel superior”`, `Jenkins` ya conoce la
instalación de `Maven` configurada previamente, por lo que solo necesitamos escribir el goal (sin el `mvn`):

````bash
test
````

### 🏗️ Añadiendo el paso de ejecución de tests

Ingresamos al job `java_app_con_maven` y en la sección `Build Steps` agregamos un nuevo paso:

- `+ Añadir un nuevo paso` → `Ejecutar tareas 'maven' de nivel superior`
...
📌 Recordemos que en la lección anterior habíamos colocado el `Goal`: `-B clean package -DskipTests`.
...
### 📋 Verificando la ejecución en la consola
...
La salida anterior confirma que las pruebas unitarias de la aplicación se ejecutaron sin errores.

### 📊 Publicar resultados de pruebas unitarias en Jenkins

Para visualizar los reportes de pruebas directamente en la interfaz de `Jenkins`, añadiremos una acción de publicación.

#### 📝 Configurar la publicación del reporte JUnit

Dentro del job, al final de la configuración:

- `+ Añadir una acción` → `Publicar los resultados de tests JUnit`
...
#### 🔍 Verificación dentro del contenedor Jenkins

Confirmamos la existencia del archivo:

````bash
D:\programming\jenkins\jenkins-guide (section-07)
λ docker container exec -it c-jenkins /bin/sh
$ cd /var/jenkins_home/workspace/java_app_con_maven
$ ls -l
total 44
drwxr-xr-x 2 jenkins jenkins 4096 Nov 14 15:57 DSL
...
### 📈 Visualizando reportes en Jenkins

Construimos nuevamente el job y recargamos la página. Aparecerá automáticamente un nuevo panel lateral mostrando:
...
Ejecutando dos o más builds adicionales, el gráfico comenzará a completarse:
