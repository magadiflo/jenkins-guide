...
4. Deberías ver: `magadiflo/spring-boot-jenkins-app` con los tags `latest` y `2` (o el número de build)

![15.png](assets/section-09/15.png)

## 🧪 Probar la imagen desde Docker Hub

Antes de ejecutar nuestra aplicación, revisemos las imágenes disponibles actualmente en nuestra Docker Host:

````bash
D:\programming\jenkins\jenkins-guide (section-09)
$ docker image ls
REPOSITORY                          TAG                        IMAGE ID       CREATED             SIZE
magadiflo/spring-boot-jenkins-app   2                          e2731189f769   About an hour ago   229MB
magadiflo/spring-boot-jenkins-app   latest                     e2731189f769   About an hour ago   229MB
jenkins                             latest                     cb83a20c9055   2 days ago          697MB
jenkins/jenkins                     2.535-jdk21                84444d75a07c   2 weeks ago         491MB
````

Como se puede observar, ya contamos con la imagen `magadiflo/spring-boot-jenkins-app` en nuestra máquina host.
Esto ocurre gracias a la configuración que hicimos previamente con `Docker-outside-of-Docker (DooD)`, que permite que
el contenedor de Jenkins construya imágenes utilizando el `Docker Engine` del host.

💡 Nota:
> Al tener la imagen localmente, Docker `no volverá a descargarla desde Docker Hub`, por lo que podemos ejecutar la
> aplicación directamente.

### 🚀 Ejecutar el contenedor de la aplicación

Levantamos un contenedor usando el tag 2

````bash
$ docker container run -d -p 8081:8081 --rm --name c-spring-boot-jenkins-app magadiflo/spring-boot-jenkins-app:2
54c41eca7b2eccfad44a9c8d9f45f1d53a03535fe88cca03caa02816c644ceb3 
````
...
54c41eca7b2e   magadiflo/spring-boot-jenkins-app:2   "java -jar app.jar"      8 seconds ago   Up 7 seconds       0.0.0.0:8081->8081/tcp, [::]:8081->8081/tcp                                                    c-spring-boot-jenkins-app
4f3d6c1a282c   jenkins                               "/usr/bin/tini -- /u…"   2 days ago      Up About an hour   0.0.0.0:8080->8080/tcp, [::]:8080->8080/tcp, 0.0.0.0:50000->50000/tcp, [::]:50000->50000/tcp   c-jenkins 
````

El contenedor aparece en estado UP, lo que indica que la aplicación está corriendo correctamente.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Thu, 20 Nov 2025 16:42:54 GMT
<
{
  "message": "Hola desde Spring Boot + Jenkins!",
  "version": "1.0.0",
  "timestamp": "2025-11-20T16:42:54.295918899"
} 
````

### 🩺 Verificar el estado de la aplicación con Actuator
...
< Content-Type: application/vnd.spring-boot.actuator.v3+json
< Transfer-Encoding: chunked
< Date: Thu, 20 Nov 2025 16:43:00 GMT
<
{
  "status": "UP"
}
...
- La imagen Docker generada por Jenkins dentro del contenedor fue creada correctamente.
- La imagen se encuentra disponible tanto en el host como en Docker Hub.
- El contenedor se ejecuta sin problemas.
- La API y los endpoints de Actuator funcionan correctamente.

Con esto validamos que el flujo completo —compilar, construir imagen y publicarla en Docker Hub— fue exitoso.
