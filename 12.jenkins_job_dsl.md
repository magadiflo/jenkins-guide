# 🚀 Sección 12: Jenkins Job DSL

---

## 🧩 Introducción al Job DSL

El término `DSL (Domain Specific Language)` hace referencia a un lenguaje diseñado para resolver problemas dentro
de un dominio específico. En el caso de `Jenkins`, el `Job DSL` permite
`definir, crear y actualizar jobs usando código`, en lugar de configurarlos manualmente desde la interfaz web.

Esto es especialmente útil en empresas donde se requiere:

- Estándares de configuración entre cientos de jobs.
- Versionar pipelines y plantillas.
- Automatizar despliegues de la infraestructura de CI/CD.

### 🛠️ ¿Qué es el plugin Jenkins Job DSL?

El plugin `Jenkins Job DSL` permite generar jobs mediante archivos escritos en `Groovy` (*`Groovy` es un lenguaje de
scripting dinámico para la plataforma Java*). Con este plugin podemos
escribir código que describa:

- Freestyle jobs
- Pipeline jobs
- Vistas
- Carpetas
- Configuraciones completas de la interfaz

De esta manera, Jenkins se convierte en un sistema declarativo, donde el código es la fuente de verdad para los jobs.

> 💡 Para poder usar `Job DSL` es indispensable instalar el plugin oficial: `Job DSL Plugin`

### 🧵 ¿Por qué usar Job DSL? (Beneficios Reales)

Los usuarios pueden describir sus jobs mediante scripts basados en `Groovy`, lo que habilita ventajas muy alineadas con
las prácticas modernas de CI/CD:

#### ✔️ Rollbacks seguros

Puedes revertir un cambio en un job simplemente regresando a una versión anterior del script en Git.

#### ✔️ Historial de cambios y auditoría

Cada modificación a un job queda registrada en el control de versiones (Git), no solo en la UI de Jenkins.

#### ✔️ Control de versiones

Los jobs pasan a ser infraestructura como código (IaC), facilitando:

- Revisiones por PR
- Control de calidad en cambios de pipelines
- Despliegue consistente de jobs en distintos entornos (Dev, QA, Prod)

#### ✔️ Automatización completa

Puedes crear cientos de jobs automáticamente a partir de una plantilla.

### 🏢 ¿Cómo se usa en empresas reales?

En el mundo profesional, `Job DSL` se emplea para:

- Definir plantillas estandarizadas para equipos completos.
- Crear jobs automáticamente para nuevos repositorios.
- Evitar configuraciones manuales que producen errores humanos.
- Mantener versiones claras de los pipelines (GitOps + CI/CD).
- Generar múltiples pipelines basados en parámetros (ej: microservicios).
- Reducir inconsistencias entre entornos o equipos.

💡 En entornos empresariales grandes, `Job DSL` convive con `Jenkins Pipeline` (declarativo o scripted).
Suele usarse así:

| Uso                         | Herramienta      |
|-----------------------------|------------------|
| Crear la estructura del job | Job DSL          |
| Ejecutar el pipeline real   | Jenkins Pipeline |

## ⚙️ Instalación del Plugin Job DSL

Para utilizar `Job DSL` en `Jenkins`, primero debemos instalar el plugin oficial. Este plugin nos permitirá definir
jobs mediante código `Groovy` y habilita las herramientas necesarias para generar, actualizar y gestionar
configuraciones desde scripts.

Nos vamos a `Administrar Jenkins` → `Plugins` y buscamos `Job DSL`, lo seleccionamos y procedemos a instalar.

![01.png](assets/section-12/01.png)

Seleccionamos `Reiniciar Jenkins cuando termina la instalación...` y esperamos mientras se instala.

![02.png](assets/section-12/02.png)

## 🌱 Seed Job & Description (Fundamentos del Job DSL)

Para utilizar correctamente el `Plugin Job DSL`, lo primero que debemos crear es un `Seed Job`.

Un `Seed Job` es un job normal de Jenkins (igual que los jobs de estilo libre que hemos estado creando), pero con
la particularidad de que incluye el paso `Process Job DSLs`. Este paso le permite ejecutar
`scripts DSL escritos en Groovy` para crear o actualizar otros jobs automáticamente. A partir de estos scripts es
posible:

- Crear nuevos jobs.
- Actualizar configuraciones.
- Regenerar plantillas.
- Aplicar cambios en masa.

En otras palabras:

> 🧠 El `Seed Job` es el `“job padre”` que genera otros jobs automáticamente mediante código.

### 🔍 ¿Por qué existe el Seed Job?

Porque `Jenkins`, por defecto, solo ejecuta lo que le asignamos manualmente. El `Seed Job` actúa como
“intérprete” del DSL, permitiendo:

- Automatizar creación de pipelines.
- Mantener jobs versionados.
- Facilitar la estandarización en equipos grandes.
- Evitar configuraciones manuales desde la UI.
- Regenerar jobs cuando cambian las plantillas.

> 💡 En empresas, los `Seed Jobs` suelen correr automáticamente al recibir cambios en su repositorio Git.

### 🛠️ Crear el Seed Job y configurar el DSL

Creamos un job de estilo libre llamado: `dsl-job`. Este será nuestro `Seed Job` inicial.

![03.png](assets/section-12/03.png)

Dentro del job, nos dirigimos al `Build Steps` y en `+ Añadir un nuevo paso` seleccionamos `Process Job DSLs`.

![04.png](assets/section-12/04.png)

### 📁 Opciones del procesamiento DSL

Al agregar el paso, aparecen dos opciones:

- 🔸 Opción 1: `Use the provided DSL script`.
    - Permite escribir el código directamente en la interfaz del job.
    - Suele usarse solo para pruebas o demos.

- 🔸 Opción 2: `Look on Filesystem`
    - Permite cargar uno o varios archivos `.groovy` desde el workspace del job.
    - Esta es la forma usada en empresas, pues el DSL se versiona en Git.

En este punto usaremos la `opción 1` para una demostración.

![05.png](assets/section-12/05.png)

### 📝 Ejemplo básico de DSL Script

Al seleccionar la `opción 1`, aparece un cuadro de texto para escribir el script. Usaremos este ejemplo simple:

````bash
job('ejemplo-job-DSL') {
  
}
````

Este script indica:

> 🔨 Crear un nuevo job llamado `ejemplo-job-DSL` utilizando como base este seed job (`dsl-job`).

Nota:
> En el siguiente enlace https://jenkinsci.github.io/job-dsl-plugin podemos ver información sobre `Jenkins Job DSL API`


![06.png](assets/section-12/06.png)

### ▶️ Ejecutando el Job DSL

Guardamos el Seed Job y luego hacemos clic en: `Construir Ahora`. Sin embargo, veremos que aparece un error:

![07.png](assets/section-12/07.png)

### 🛡️ ¿Por qué aparece el error?

Porque `Jenkins`, por seguridad, `no ejecuta scripts Groovy no aprobados`. `Jenkins` requiere revisar los scripts
para evitar ejecución de código potencialmente peligroso.

Debemos aprobarlos manualmente:

````
Administrar Jenkins → In-process Script Approval 
````

![08.png](assets/section-12/08.png)

### ✔️ Aprobando el script

Entramos a `In-Process Script Approval` y hacemos clic en: `Approve`.

![09.png](assets/section-12/09.png)

Después de aprobarlo, volvemos a ejecutar el Seed Job y veremos que ahora el build finaliza correctamente:

![10.png](assets/section-12/10.png)

### 🎉 Verificando que el job fue creado

Vamos al panel principal de Jenkins y veremos que se generó un nuevo job: `ejemplo-job-DSL`.

![11.png](assets/section-12/11.png)

Y si ingresamos al job recién creado, veremos la referencia: `Seed job: dsl-job`. Esto nos confirma que el job fue
creado mediante código usando nuestro Seed Job.

![12.png](assets/section-12/12.png)

## 📁 Source Control Management (SCM)

En esta lección aprenderemos a integrar `GitHub` dentro de un `Job` creado mediante `Jenkins Job DSL`, indicándole
desde qué repositorio debe clonar el código fuente para ejecutar el pipeline o los scripts asociados.

Integrar correctamente `SCM` es fundamental en entornos empresariales, ya que:

- Permite ejecutar pipelines reproducibles.
- Garantiza que el código provenga siempre de una fuente controlada.
- Hace posible trabajar con branching models modernos (`GitFlow`, `trunk-based development`).
- Facilita auditorías y trazabilidad.

### 🔍 Conociendo el Git SCM Source en Job DSL

Para definir el origen del código dentro del script DSL, utilizamos el bloque `scm { ... }`.

En la documentación oficial del plugin:

➡️ [Jenkins Job DSL Plugin](https://jenkinsci.github.io/job-dsl-plugin/#method/javaposse.jobdsl.dsl.helpers.ScmContext.git)
encontraremos ejemplos como este:

![13.png](assets/section-12/13.png)

En la imagen vemos el `example-3`, mismo que se representa a continuación.

````bash
// add user name and email options
job('example-3') {
    scm {
        git('git@git') { node -> // is hudson.plugins.git.GitSCM
            node / gitConfigName('DSL User')
            node / gitConfigEmail('me@me.com')
        }
    }
} 
````

#### 📝 ¿Qué son `gitConfigName` y `gitConfigEmail`?

- Estos parámetros dentro del bloque `scm` están configurando la identidad que `Git` utilizará para firmar cualquier
  nuevo commit que Jenkins haga en el repositorio.
- Esto es el equivalente a ejecutar los siguientes comandos de Git dentro del espacio de trabajo del job de Jenkins (si
  el job hiciera un commit):
    ````bash
    git config user.name "DSL User"
    git config user.email "me@me.com" 
    ````
- `¿Son importantes?` Sí, porque si tu job realiza una acción que resulta en un commit (por ejemplo, actualizando un
  número de versión o un archivo de configuración), esta es la información del autor que aparecerá en el historial de
  Git.

### 🧪 Ejemplo previo usado en el curso

Anteriormente, probamos este script desde un job parametrizable de tipo Freestyle llamado `Parámetro booleano en Job`:

````bash
#!/bin/bash
echo "Bienvenido al planeta ${PLANET}"
if [ "${AGENT}" = "true" ] 
then
	echo "Prepárese para la misión agente X, que la fuerza te acompañe."
else
	echo "Disfruta el planeta humano ${NAME}, trata de no morir"
fi
echo "..."
sleep 10
echo "¡Suerte! La necesitarás." 
````

Este mismo script estará subido en el repositorio de GitHub del tutor en un archivo llamado `jobscript.sh`, tal como
se muestra a continuación.

![14.png](assets/section-12/14.png)

### 🛠️ Realizando el Ejemplo con Job DSL

Nos dirigimos al `Seed Job` `dsl-job` → `Configure` → `Build Steps` → `Process Job DSLs`, e insertamos lo siguiente:

````bash
job('ejemplo-job-DSL') {
  description('Job DSL de ejemplo para el curso de Jenkins')
  scm {
        git('https://github.com/macloujulian/jenkins.job.parametrizado.git', 'main') { node ->
            node / gitConfigName('Martín Díaz')
            node / gitConfigEmail('magadiflo@gmail.com')
        }
    }
}
````

#### 💡 ¿Qué hace este script?

- Crea un nuevo job llamado `ejemplo-job-DSL`.
- Configura `Git SCM` apuntando al repositorio `GitHub`.
- Selecciona la rama `main`.
- Define el nombre y correo del autor si Jenkins ejecuta operaciones Git.

En la siguiente imagen se observa la modificación anterior.

![15.png](assets/section-12/15.png)

Luego de dar en `Save`, vamos a `Construir ahora`. Probablemente, el job no se ejecute y nos muestre un mensaje
de error `ERROR: script not yet approved for use`.

### 🔐 Mensaje de error: `script not yet approved for use`

El error `ERROR: script not yet approved for use` significa que el Groovy Sandbox de Jenkins está bloqueando la
ejecución del script porque contiene métodos o clases que Jenkins considera potencialmente inseguros.

Cuando ejecutas un Seed Job que usa Job DSL, en realidad estás ejecutando un script Groovy. Jenkins tiene un mecanismo
de seguridad (el `In-process Script Approval`) para evitar que scripts maliciosos puedan dañar el sistema de archivos o
acceder a información sensible.

Para resolver esto, debemos aprobar manualmente el script del Job DSL.

![16.png](assets/section-12/16.png)

### ✔️ Ejecución Exitosa

Cuando el Seed Job corre nuevamente:

- El script queda aprobado.
- El job `ejemplo-job-DSL` se crea correctamente.
- La configuración SCM aparece con el repositorio y la rama definida.

![17.png](assets/section-12/17.png)

Dentro de este `Seed Job` llamado `dls-job` vemos un job `ejemplo-job-DSL`, ingresamos en él y vemos que `Git`
se ha configurado indicando el `repositorio de GitHub` y la rama `main`.

![18.png](assets/section-12/18.png)

Recordemos que el `Process Job DSLs` inicia con `job('ejemplo-job-DSL'){...}`, por eso es que la configuración de git
y el repositorio de GitHub se aplicaron a ese job.

### 📌 Conclusión

- El bloque `scm` en Job DSL permite integrar repositorios GitHub directamente en la definición de jobs.
- Es fundamental configurar nombre y email para que los commits generados por Jenkins tengan identidad clara.
- En Jenkins, los scripts DSL son Groovy y requieren aprobación de seguridad antes de ejecutarse.
- Una vez aprobado, el job queda listo para usar el código fuente del repositorio remoto.

## 🎛️ Parámetros en Jenkins DSL (Job DSL)

En esta lección aprenderemos cómo definir parámetros en un Job utilizando `Jenkins Job DSL`.
Los parámetros permiten que un job sea dinámico, configurable y reutilizable; son ampliamente usados en entornos
reales para:

- Deploys a distintos entornos (`DEV`, `QA`, `PROD`).
- Activar o desactivar funcionalidades.
- Pasar variables a scripts (como nuestro `jobscript.sh`).
- Seleccionar versiones, ramas o artefactos.
- Cambiar comportamientos del pipeline según valores de entrada.

### 📚 Consultando documentación oficial

En la documentación del plugin:
➡️ [Jenkins Job DSL Plugin](https://jenkinsci.github.io/job-dsl-plugin/#method/javaposse.jobdsl.dsl.jobs.FreeStyleJob.parameters)

Encontramos el siguiente ejemplo básico:

````bash
job('example') {
    parameters {
        booleanParam('FLAG', true)
        choiceParam('OPTION', ['option 1 (default)', 'option 2', 'option 3'])
    }
}
````

💡 ¿Qué hace este ejemplo?

- `booleanParam` → crea un checkbox
- `choiceParam` → crea una lista desplegable con opciones

Este patrón es el que usaremos para definir nuestros parámetros personalizados.

![19.png](assets/section-12/19.png)

### 🛠️ Configurando parámetros para el ejemplo real del curso

Nuestro script `jobscript.sh` descargado desde GitHub requiere los parámetros:

- NAME
- PLANET
- AGENT

Por lo tanto, debemos agregar esos parámetros mediante el `DSL`. Volvemos a nuestro
`Seed Job` → `Configure` → `Process Job DSLs` y agregamos los parámetros dentro del job `ejemplo-job-DSL`.

````bash
job('ejemplo-job-DSL') {
  description('Job DSL de ejemplo para el curso de Jenkins')
  scm {
        git('https://github.com/macloujulian/jenkins.job.parametrizado.git', 'main') { node ->
            node / gitConfigName('Martín Díaz')
            node / gitConfigEmail('magadiflo@gmail.com')
        }
    }
  parameters {
    stringParam('NAME', 'Milagritos', 'Parámetro de cadena para el Job Booleano')
    choiceParam('PLANET', ['Mercurio', 'Venus', 'Tierra', 'Marte', 'Júpiter', 'Saturno', 'Urano', 'Neptuno', 'Plutón'], 'Lista de parámetros')
    booleanParam('AGENT', false)
  }
} 
````

![20.png](assets/section-12/20.png)

Damos en `Save` y vamos a `Construir ahora`. Si nos aparece el error `ERROR: script not yet approved for use`
ya sabemos que debemos aprobar la ejecución de ese job en:
`Administrar Jenkins` > `In-process Script Approval`.

### 🖼️ Configuración visual en la UI

Al ejecutar el `Seed Job` y después ir al `ejemplo-job-DSL`, veremos los parámetros listos para ser usados.

![21.png](assets/section-12/21.png)

Esto confirma que el DSL del `Seed Job` está creando correctamente los parámetros en el job destino.

### ⚙️ ¿Por qué los parámetros aparecen en `ejemplo-job-DSL`?

Porque el Seed Job ejecuta:

````bash
job('ejemplo-job-DSL') { ... } 
````

Todo lo que esté dentro de ese bloque se aplica `al job que estamos declarando` y no al `Seed Job`.
El Seed Job solo sirve para `crear o actualizar` otros jobs. Este patrón es estándar en grandes empresas:

#### 🧩 Patrón típico en entornos reales

- `Seed Job`: crea/actualiza cientos de jobs automáticamente.
- `Jobs generados por DSL`: contienen toda la configuración (SCM, parámetros, triggers, etc.).
- La configuración vive en código, no en clics manuales.

### Verifica parámetros en job `ejemplo-job-DSL`

Si damos clic en `Build with Parameters` en el job `ejemplo-job-DSL`, vemos que nos aparece los parámetros configurados.

![22.png](assets/section-12/22.png)

## ⏱️ Triggers & Wrappers en Jenkins Job DSL

En esta lección aprenderemos a utilizar dos conceptos fundamentales cuando definimos jobs mediante `Jenkins Job DSL`:

- `Wrappers` → Acciones adicionales antes/después del job.
- `Triggers` → Programar la ejecución de un job automáticamente.

Ambos son utilizados en entornos empresariales para controlar el comportamiento de los jobs y automatizar procesos de
CI/CD.

### 🎁 Wrappers

Un `wrapper` es un bloque que permite agregar comportamientos adicionales al job, ya sea antes o después de la
ejecución.

Ejemplos típicos en empresas:

- ✔️ Configurar timeout.
- ✔️ Ejecutar el job en un entorno controlado.
- ✔️ Agregar timestamps a la consola.
- ✔️ Activar ansiColor.
- ✔️ Enmascarar logs con credenciales.

Aunque lo estudiaremos más adelante con profundidad, podemos describirlo así:

> 💡 `Wrapper` = comportamiento extra antes o después de la ejecución del job.

Ejemplo real:

````groovy
wrappers {
    timeout {
        absolute(10) // falla el job si dura más de 10 minutos
    }
    timestamps() // agrega tiempo a cada línea del log
} 
````

### ⏱️ Triggers en Jenkins

Los `triggers` permiten ejecutar un job de manera automática sin intervención humana.

En el mundo real esto se usa para:

- Ejecutar builds diarios o nocturnos (nightly builds).
- Validar ramas periódicamente.
- Sincronizar repositorios.
- Generar reportes automáticos.
- Automatizar tareas repetitivas.

En `Job DSL`, un trigger común es el `cron`.

### 🧩 ¿Qué es un cron?

Un `cron` es una expresión que indica cuándo debe ejecutarse algo. Tiene 5 campos:

````scss
┌────────── Minutos (0 - 59)
│ ┌──────── Hora (0 - 23)
│ │ ┌────── Día del mes (1 - 31)
│ │ │ ┌──── Mes (1 - 12)
│ │ │ │ ┌── Día de la semana (0 - 7)
│ │ │ │ │
│ │ │ │ │
* * * * * 
````

Ejemplos:

| Expresión   | Significa                                |
|-------------|------------------------------------------|
| `* * * * *` | Cada minuto                              |
| `0 * * * *` | Cada hora exacta                         |
| `0 0 * * *` | Todos los días a medianoche              |
| `H * * * *` | Una vez por hora, usando hashing estable |

#### 💡 `H` en Jenkins

`Jenkins` incluye la letra `H (hash)` para distribuir automáticamente las ejecuciones y evitar que todos los
jobs corran a la vez. Es una mejor práctica recomendada en empresas.

Ejemplo:

````bash
H/7 * * * *
````

➡️ `Ejecuta el job cada 7 minutos`, pero no siempre en el mismo minuto exacto. Esto balancea la carga del servidor.

### 🧩 Nota rápida sobre el formato `cron` en otros sistemas

En `Apache Camel` los `cron` usan `6 campos` porque se basan en `Quartz Cron`, que incluye el campo de segundos:

````bash
Segundos  Minutos  Horas  DíaMes  Mes  DíaSemana 
````

En `Jenkins`, en cambio, el `cron` usa el formato `Unix clásico` de `5 campos`, sin segundos:

````bash
Minutos  Horas  DíaMes  Mes  DíaSemana
````

💡 Por eso en `Camel` verás `expresiones con 6 asteriscos`, mientras que en `Jenkins solo 5`.

### 🛠️ Trigger en Job DSL

Así se define un trigger con cron:

````groovy
triggers {
    cron('H/7 * * * *')
} 
````

### 🧱 DSL Completo con SCM, Parámetros y Trigger

Aquí está el script completo utilizado en esta lección:

````groovy
job('ejemplo-job-DSL') {
    description('Job DSL de ejemplo para el curso de Jenkins')
    scm {
        git('https://github.com/macloujulian/jenkins.job.parametrizado.git', 'main') { node ->
            node / gitConfigName('Martín Díaz')
            node / gitConfigEmail('magadiflo@gmail.com')
        }
    }
    parameters {
        stringParam('NAME', 'Milagritos', 'Parámetro de cadena para el Job Booleano')
        choiceParam('PLANET', ['Mercurio', 'Venus', 'Tierra', 'Marte', 'Júpiter', 'Saturno', 'Urano', 'Neptuno', 'Plutón'], 'Lista de parámetros')
        booleanParam('AGENT', false)
    }
    triggers {
        cron('H/7 * * * *') // Se ejecuta cada 7 minutos
    }
}
````

La imagen muestra lo mismo que el código anterior.

![23.png](assets/section-12/23.png)

### 🖼️ Visualización en Jenkins

Luego de ejecutar el `Seed Job` (recordemos aprobar el script en `Manage Jenkins` → `In-process Script Approval`).

![24.png](assets/section-12/24.png)

El job generado `ejemplo-job-DSL` mostrará el trigger configurado:

![25.png](assets/section-12/25.png)

Como podemos observar, en la opción `Triggers` se muestra el crón definido anteriormente. Este trigger se ejecutará
cada 7 minutos.

## 🚀 Steps (Build Steps en Job DSL)

Los `Steps` permiten definir qué acciones ejecutará un job durante su fase de construcción. Son equivalentes a los
`“Build Steps”` que vemos al crear un `job Freestyle` desde la interfaz de `Jenkins`, pero expresados como código
dentro del `Job DSL`.

### 🧱 ¿Qué son los Steps?

Los `steps` son `instrucciones de ejecución` que Jenkins corre dentro del job. Pueden ser:

- 🖥️ Comandos de shell/batch.
- 🔧 Construcciones con herramientas (Maven, Gradle, Ant).
- 📦 Scripts almacenados en el repositorio.
- 🚀 Invocaciones de pipelines, scripts Groovy, etc.

Ejemplo básico desde la documentación

````groovy
job('example') {
    steps {
        shell('echo START')
        gradle('check')
    }
}
````

✔️ En este ejemplo se ejecuta un comando shell y luego una tarea de Gradle.

#### 📚 Consultando documentación oficial

La referencia completa se encuentra aquí:
[🔗 Job DSL Plugin › FreeStyleJob › steps](https://jenkinsci.github.io/job-dsl-plugin/#method/javaposse.jobdsl.dsl.jobs.FreeStyleJob.steps)

![26.png](assets/section-12/26.png)

### 🛠️ Agregando Steps en nuestro Seed Job dsl-job

Vamos a actualizar el DSL para que el job generado (`ejemplo-job-DSL`) ejecute un script del repositorio:

📍 `dsl-job` → `Configure` → `Build Steps` → `Process Job DSLs`

Agregamos:

````groovy
job('ejemplo-job-DSL') {
    description('Job DSL de ejemplo para el curso de Jenkins')
    scm {
        git('https://github.com/macloujulian/jenkins.job.parametrizado.git', 'main') { node ->
            node / gitConfigName('Martín Díaz')
            node / gitConfigEmail('magadiflo@gmail.com')
        }
    }
    parameters {
        stringParam('NAME', 'Milagritos', 'Parámetro de cadena para el Job Booleano')
        choiceParam('PLANET', ['Mercurio', 'Venus', 'Tierra', 'Marte', 'Júpiter', 'Saturno', 'Urano', 'Neptuno', 'Plutón'], 'Lista de parámetros')
        booleanParam('AGENT', false)
    }
    triggers {
        cron('H/7 * * * *')
    }
    steps {
        shell('bash jobscript.sh')
    }
}
````

### 📝 ¿Qué hace este Step?

````groovy
shell('bash jobscript.sh')
````

Este comando:

- Invoca el shell local del agente donde corre el job.
- Ejecuta el archivo `jobscript.sh` presente en el repositorio clonado.

💡 En entornos reales, siempre se acostumbra:

- Validar permisos de ejecución (chmod +x jobscript.sh)
- Evitar comandos inline largos.
- Mantener los scripts versionados dentro del repo (como hacemos aquí).

![27.png](assets/section-12/27.png)

### ▶️ Ejecutando el Seed Job

Después de hacer clic en `Construir ahora`, Jenkins podría requerir aprobación del script:

📍 `Manage Jenkins` → `In-process Script Approval`

Una vez aprobado, la segunda ejecución debe completarse sin errores:

![28.png](assets/section-12/28.png)

### 🔍 Verificando el Step en el job generado

Ahora revisamos el job creado `ejemplo-job-DSL`:

📍 `ejemplo-job-DSL` → `Configure` → `Build Steps`

Verás reflejado el comando que definimos en el DSL:

![29.png](assets/section-12/29.png)

## 📦 Publishers — Acciones posteriores a la construcción

Los `Publishers` son acciones que Jenkins ejecuta después de construir un job. Se utilizan típicamente para:

- 📤 Publicar artefactos.
- 🧪 Publicar reportes de pruebas.
- ✉️ Enviar notificaciones (email, Slack, Teams, etc.).
- 📁 Copiar archivos a otro job o servidor.
- 🔗 Integrar resultados con plugins externos.

Son equivalentes a las `“Acciones post-ejecución”` que vemos en un `job Freestyle`, pero expresadas como código
declarativo dentro del Job DSL.

#### 🔧 Ejemplo básico desde la documentación

````groovy
job('example') {
    publishers {
        archiveArtifacts('build/test-output/**/*.html')
        archiveJunit('**/target/surefire-reports/*.xml')
    }
}
````

✔️ En este ejemplo publicamos reportes HTML y resultados JUnit.

#### 📚 Consultando documentación oficial

La referencia completa se encuentra aquí:
🔗 [Job DSL Plugin › FreeStyleJob › publishers](https://jenkinsci.github.io/job-dsl-plugin/#method/javaposse.jobdsl.dsl.jobs.FreeStyleJob.publishers)

![30.png](assets/section-12/30.png)

### 🛠️ Agregando Publishers en nuestro Seed Job dsl-job

Vamos a actualizar el DSL para que el job generado (`ejemplo-job-DSL`) tenga dos tipos de notificaciones:

- 📧 Correo electrónico (Mailer)
- 💬 Slack (Slack Notifier Plugin)

📍 `dsl-job` → `Configure` → `Build Steps` → `Process Job DSLs`

````groovy
job('ejemplo-job-DSL') {
    description('Job DSL de ejemplo para el curso de Jenkins')
    scm {
        git('https://github.com/macloujulian/jenkins.job.parametrizado.git', 'main') { node ->
            node / gitConfigName('Martín Díaz')
            node / gitConfigEmail('magadiflo@gmail.com')
        }
    }
    parameters {
        stringParam('NAME', 'Milagritos', 'Parámetro de cadena para el Job Booleano')
        choiceParam('PLANET', ['Mercurio', 'Venus', 'Tierra', 'Marte', 'Júpiter', 'Saturno', 'Urano', 'Neptuno', 'Plutón'], 'Lista de parámetros')
        booleanParam('AGENT', false)
    }
    triggers {
        cron('H/7 * * * *')
    }
    steps {
        shell('bash jobscript.sh')
    }
    publishers {
        // 📧 Email Notifications
        mailer('magadiflo@gmail.com', true, true)

        // 💬 Slack Notifications
        slackNotifier {
            notifyAborted(true)
            notifyEveryFailure(true)
            notifyNotBuilt(false)
            notifyUnstable(false)
            notifyBackToNormal(true)
            notifySuccess(false)
            notifyRepeatedFailure(false)
            startNotification(false)
            includeTestSummary(false)
            includeCustomMessage(false)
            customMessage(null)
            sendAs(null)
            commitInfoChoice('NONE')
            teamDomain(null)
            authToken(null)
        }
    }
}
````

### 📤 Explicación rápida de los Publishers utilizados

- 📧 `mailer(...)`
    ````groovy
    mailer(String recipients, Boolean dontNotifyEveryUnstableBuild, Boolean sendToIndividuals)
    ````
    - Envía correo al finalizar la construcción.
    - Es parte de la documentación oficial del DSL.
    - Es útil en entornos pequeños o para notificaciones simples.


- 💬 `slackNotifier { ... }`
    - Este no aparece en la documentación oficial del Job DSL, porque depende del plugin:
      `➡️ Slack Notification Plugin`.
    - Las propiedades mostradas incluyen opciones para notificar por fallas, abortos, inestabilidad, recuperación, etc.
    - 💡 En empresas reales, Slack es uno de los canales de notificación más usados porque permite:
        - Alertas en tiempo real.
        - Integración con canales y equipos.
        - Control granular del nivel de ruido.

![31.png](assets/section-12/31.png)

### ▶️ Ejecutando el Seed Job

Después de hacer clic en `Construir ahora`, `Jenkins` podría requerir aprobación del script:

📍 `Manage Jenkins` → `In-process Script Approval`

Una vez aprobado, la segunda ejecución debe completarse sin errores:

![32.png](assets/section-12/32.png)

### 🔍 Verificando el Step en el job generado

Ahora revisamos el job creado `ejemplo-job-DSL`:

📍 `ejemplo-job-DSL` → `Configure` → `Acciones para ejecutar después.`

Debe aparecer la configuración reflejada tal como la definimos:

![33.png](assets/section-12/33.png)

## 🚀 Construyamos el ejemplo Job DSL

Hasta este punto hemos creado nuestro Seed Job `dsl-job`, el cual contiene en su sección
`Build Steps` → `Process Job DSLs` el siguiente `DSL Script`:

````groovy
job('ejemplo-job-DSL') {
    description('Job DSL de ejemplo para el curso de Jenkins')
    scm {
        git('https://github.com/macloujulian/jenkins.job.parametrizado.git', 'main') { node ->
            node / gitConfigName('Martín Díaz')
            node / gitConfigEmail('magadiflo@gmail.com')
        }
    }
    parameters {
        stringParam('NAME', 'Milagritos', 'Parámetro de cadena para el Job Booleano')
        choiceParam('PLANET', ['Mercurio', 'Venus', 'Tierra', 'Marte', 'Júpiter', 'Saturno', 'Urano', 'Neptuno', 'Plutón'], 'Lista de parámetros')
        booleanParam('AGENT', false)
    }
    triggers {
        cron('H/7 * * * *')
    }
    steps {
        shell('bash jobscript.sh')
    }
    publishers {
        mailer('magadiflo@gmail.com', true, true)
        slackNotifier {
            notifyAborted(true)
            notifyEveryFailure(true)
            notifyNotBuilt(false)
            notifyUnstable(false)
            notifyBackToNormal(true)
            notifySuccess(false)
            notifyRepeatedFailure(false)
            startNotification(false)
            includeTestSummary(false)
            includeCustomMessage(false)
            customMessage(null)
            sendAs(null)
            commitInfoChoice('NONE')
            teamDomain(null)
            authToken(null)
        }
    }
}
````

Recordemos que un `Seed Job` es un `job padre` que genera otros jobs automáticamente en base al código definido
en el `DSL Script`. En este caso, dicho script crea un job llamado `ejemplo-job-DSL`.

#### 📝 ¿Qué hace cada sección del DSL Script?

- `job('ejemplo-job-DSL')` → define el nombre del job que será generado.
- `description(...)` → agrega una descripción visible en la configuración del job.
- `scm { git(...) }` → clona un repositorio GitHub y configura nombre/email del autor para operaciones Git.
- `parameters { ... }` → define los parámetros que el usuario deberá ingresar al ejecutar el job.
- `triggers { cron(...) }` → programa el job para ejecutarse cada cierto tiempo (en este caso, cada 7 minutos).
- `steps { shell('bash jobscript.sh') }` → ejecuta un script shell descargado desde el repositorio.
- `publishers { ... }` → configura acciones post-ejecución: envío de correo y notificación a Slack.

Esta es la versión compacta de lo que hemos construido paso a paso.

### ▶️ Ejecutando el Seed Job

Después de hacer clic en `Construir ahora`, `Jenkins` podría requerir aprobación del script:

📍 `Manage Jenkins` → `In-process Script Approval`

Una vez aprobado, ejecutamos nuevamente el `dsl-job` y esta vez la ejecución finalizará correctamente.

![34.png](assets/section-12/34.png)

### 📌 Verificando construcción del job `ejemplo-job-DSL`

Si volvemos al menú principal de `Jenkins` veremos que el nuevo job ha sido creado exitosamente a partir del `Seed Job`:

![35.png](assets/section-12/35.png)

### ▶️ Ejecutando el job `ejemplo-job-DSL`

Este job requiere parámetros, por eso al ejecutarlo Jenkins mostrará el formulario correspondiente.

![36.png](assets/section-12/36.png)

Después de ingresar los valores, iniciamos la ejecución.

### 📄 Log de construcción del job generado

````bash
Started by user System Admin
Running as SYSTEM
Building in workspace /var/jenkins_home/workspace/ejemplo-job-DSL
The recommended git tool is: NONE
No credentials specified
 > git rev-parse --resolve-git-dir /var/jenkins_home/workspace/ejemplo-job-DSL/.git # timeout=10
Fetching changes from the remote Git repository
 > git config remote.origin.url https://github.com/macloujulian/jenkins.job.parametrizado.git # timeout=10
Fetching upstream changes from https://github.com/macloujulian/jenkins.job.parametrizado.git
 > git --version # timeout=10
 > git --version # 'git version 2.47.3'
 > git fetch --tags --force --progress -- https://github.com/macloujulian/jenkins.job.parametrizado.git +refs/heads/*:refs/remotes/origin/* # timeout=10
 > git rev-parse origin/main^{commit} # timeout=10
Checking out Revision 0723ccc1532e5e3875e3a10e603496293deee91f (origin/main)
 > git config core.sparsecheckout # timeout=10
 > git checkout -f 0723ccc1532e5e3875e3a10e603496293deee91f # timeout=10
Commit message: "jobscript.sh"
 > git rev-list --no-walk 0723ccc1532e5e3875e3a10e603496293deee91f # timeout=10
 > git tag -a -f -m Jenkins Build #4 jenkins-ejemplo-job-DSL-4 # timeout=10
[ejemplo-job-DSL] $ /bin/sh -xe /tmp/jenkins12978913452277900551.sh
+ bash jobscript.sh
Bienvenido/a al planeta 
Disfruta el planeta humano/a , trata de no morir.
...
¡Suerte! La necesitarás.
[Slack Notifications] found #3 as previous completed, non-aborted build
Finished: SUCCESS 
````

Como se aprecia, el script `jobscript.sh` se ejecutó correctamente y las notificaciones configuradas también se
dispararon.

![37.png](assets/section-12/37.png)

## 📄 Documentación de Error: Fallo de Clonación de Git en Jobs de Jenkins

### 🚨 Error Identificado: Fallo al Obtener Repositorio Remoto

Este error ocurre durante la fase de `Source Code Management (SCM)`, específicamente cuando el `Job` de `Jenkins`
intenta clonar o actualizar el repositorio Git.

### Mensaje Típico en la Consola

El error se manifiesta con mensajes relacionados con la corrupción o falta de un objeto Git en la caché local:

````bash
ERROR: Error fetching remote repo 'origin'
...
Caused by: hudson.plugins.git.GitException: Command "git fetch..." returned status code 1:
stderr: fatal: bad object refs/tags/jenkins-ejemplo-job-DSL-19
error: https://aws.amazon.com/es/what-is/repo/ did not send all necessary objects 
````

### Causa Raíz

La causa más común es que la `copia local del repositorio (el Workspace)` del Job en el servidor de Jenkins se ha
corrompido, o contiene una referencia Git obsoleta/incorrecta a una etiqueta o un commit que el servidor remoto (como
GitHub) ya no reconoce o no puede enviar correctamente.

### ✅ Solución Aplicada: Limpiar Zona de Trabajo

La solución más rápida y directa para este problema, sin necesidad de modificar el repositorio de Git o la configuración
del Job DSL, es forzar a Jenkins a realizar una clonación completamente nueva y limpia.

| Paso | Acción               | Descripción                                                                                                                                                             |
|------|----------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1.   | Navegar al Job       | Ir al Job específico de Jenkins que está fallando (ej. `ejemplo-job-DSL`).                                                                                              |
| 2.   | Limpiar el Workspace | En el menú lateral izquierdo, buscar y seleccionar la opción `"Zona de Trabajo" (Workspace)`. Dentro, seleccionar `"Limpiar Zona de trabajo" (o "Wipe Out Workspace")`. |
| 3.   | Re-ejecutar          | Construir (Ejecutar) el Job nuevamente haciendo clic en `Construir Ahora`.                                                                                              |

### Resultado Esperado

Al limpiar la `Zona de Trabajo`, Jenkins elimina la carpeta de caché corrupta
(`/var/jenkins_home/workspace/[nombre_del_job]`). En la siguiente construcción, el cliente Git realiza una clonación
fresh (fresca) del repositorio, resolviendo el problema del "bad object" y permitiendo que el Job proceda con los pasos
de ejecución (steps).

## 📦 ¿Cómo utilizar DSL desde SCM (GitHub)?

En esta lección utilizaremos nuevamente el repositorio del tutor:
🔗 https://github.com/macloujulian/jenkins.job.parametrizado

La idea principal es `versionar el código DSL` en GitHub y luego hacer que Jenkins lo lea directamente desde ese
repositorio, en lugar de escribir el `DSL` dentro del `Seed Job`.

### 📄 Creando el archivo `parametrizadoDSL.groovy` en GitHub

El tutor creó un archivo llamado `parametrizadoDSL.groovy` con la misma definición DSL que veníamos usando,
y lo subió a su repositorio. Si estuviéramos trabajando con nuestro propio repositorio, deberíamos tener allí
este archivo también.

Nuestro archivo quedaría así, con el nombre del job actualizado a `ejemplo2-job-DSL`:

````groovy
job('ejemplo2-job-DSL') {
    description('Job DSL de ejemplo para el curso de Jenkins')
    scm {
        git('https://github.com/macloujulian/jenkins.job.parametrizado.git', 'main') { node ->
            node / gitConfigName('Martín Díaz')
            node / gitConfigEmail('magadiflo@gmail.com')
        }
    }
    parameters {
        stringParam('NAME', 'Milagritos', 'Parámetro de cadena para el Job Booleano')
        choiceParam('PLANET', ['Mercurio', 'Venus', 'Tierra', 'Marte', 'Júpiter', 'Saturno', 'Urano', 'Neptuno', 'Plutón'], 'Lista de parámetros')
        booleanParam('AGENT', false)
    }
    triggers {
        cron('H/7 * * * *')
    }
    steps {
        shell('bash jobscript.sh')
    }
    publishers {
        mailer('magadiflo@gmail.com', true, true)
        slackNotifier {
            notifyAborted(true)
            notifyEveryFailure(true)
            notifyNotBuilt(false)
            notifyUnstable(false)
            notifyBackToNormal(true)
            notifySuccess(false)
            notifyRepeatedFailure(false)
            startNotification(false)
            includeTestSummary(false)
            includeCustomMessage(false)
            customMessage(null)
            sendAs(null)
            commitInfoChoice('NONE')
            teamDomain(null)
            authToken(null)
        }
    }
}
````

📌 Este archivo debe existir en la raíz del repositorio, igual que en el repositorio del tutor.

Entonces, para tener claro hasta este punto, si nosotros estuviéramos trabajando con nuestro propio repositorio,
tendríamos el archivo `parametrizadoDSL.groovy` en GitHub tal como lo tiene ahora mismo el tutor en su repositorio.

![38.png](assets/section-12/38.png)

En la siguiente imagen se muestra la raíz del repositorio del tutor donde tenemos el archivo `parametrizadoDSL.groovy`.

![39.png](assets/section-12/39.png)

### 🛠️ Creando una nueva tarea en Jenkins

Creamos una nueva tarea llamada `Job Parametrizado Nuevo DSL` del tipo estilo libre.

![40.png](assets/section-12/40.png)

Dentro de la nueva tarea creada nos vamos hacia `Configurar el origen del código fuente` y seleccionamos `Git`.
Procedemos a completar los campos:

- Repository URL: `https://github.com/macloujulian/jenkins.job.parametrizado.git`
- Branch Specifier: `main`

![41.png](assets/section-12/41.png)

Luego, nos vamos hacia `Build Steps` y en el botón `Añadir un nuevo paso` seleccionamos `Process Job DSLs`.
Luego de que nuestro repositorio sea clonado, le debemos decir a Jenkins dónde se encuentra el archivo `DSL`, así
que en el campo `DSL Scripts` le indicamos la ruta: `./parametrizadoDSL.groovy`.

![42.png](assets/section-12/42.png)

Esto le indica a Jenkins que debe procesar ese archivo después del checkout.

### ▶️ Ejecutando la nueva tarea

Procedemos a dar clic en `Construir ahora`, `Jenkins` podría requerir aprobación del script, por lo que la primera
ejecución podría fallar.

Seguimos la siguiente ruta para realizar la aprobación.

📍 `Manage Jenkins` → `In-process Script Approval`

Luego de aprobar el contenido, realizamos la segunda ejecución y debería finalizar exitosamente.

![43.png](assets/section-12/43.png)

Si ingresamos al `Console Output` de esa segunda ejecución, veremos que nos muestra `Finished: SUCCESS`.

![44.png](assets/section-12/44.png)

### 🔍 Verificando el job generado

Si la ejecución fue correcta, Jenkins habrá creado automáticamente el job definido en el archivo DSL: `ejemplo2-job-DSL`

Este job incluirá:

- Configuración SCM
- Parámetros
- Trigger CRON
- Steps
- Publishers (email + Slack)

Todo exactamente como lo definimos en el archivo `parametrizadoDSL.groovy`.

![45.png](assets/section-12/45.png)

## 📘 Construye el Job automáticamente con un push

En esta lección aprenderemos a configurar la integración entre `GitHub` → `Jenkins` para que, cada vez que ocurra
un `push` o `merge` en un repositorio, se dispare automáticamente la construcción del job en Jenkins.

Esta integración se logra gracias a un `Webhook`: `GitHub` enviará un `POST` a una URL especial que `Jenkins` expone,
y `Jenkins` procesará ese evento para construir el job de manera inmediata.

### 🧠 ¿Cómo funciona el Webhook?

El flujo es así:

- `Jenkins` expone una `URL pública` llamada `Hook URL`.
- `GitHub` envía un `POST` a esa URL cada vez que ocurre un push.
- `Jenkins` recibe la notificación y ejecuta automáticamente el job configurado.

💡 Idea clave:
> Para que `GitHub` pueda enviar la notificación, la `URL del Hook` `debe ser accesible desde internet`.  
> Un `Jenkins` ejecutado en `localhost` `no puede recibir webhooks`, porque `GitHub` no puede conectarse a tu máquina
> local.

### 🔗 1. Obteniendo la Hook URL desde Jenkins

Para conocer la URL que Jenkins utilizará como Webhook:

1. Ve a `Administrar Jenkins` → `System`.
2. Ubica la sección `GitHub`.
3. Clic en `Avanzado`.
4. Marca `Specify another hook URL for GitHub configuration`.

Aparecerá algo como:

````bash
http://localhost:8080/github-webhook/
````

![46.png](assets/section-12/46.png)

⚠️ Advertencia importante
> `GitHub` `no acepta webhooks que apunten a localhost`, ya que debe `comunicarse públicamente con Jenkins`.
> `Solo funcionará si Jenkins está en un servidor accesible desde internet` (o usando túneles tipo ngrok).

### 🍴 2. Creando un Fork del repositorio del tutor

Para continuar con la práctica, se realiza un `Fork` del repositorio del tutor:

👉 https://github.com/macloujulian/jenkins.job.parametrizado

Esto permite trabajar con un repositorio propio, donde sí podemos configurar `webhooks`.

![47.png](assets/section-12/47.png)

### ⚙️ 3. Configurando el Webhook en GitHub

En tu repositorio `forkeado`:

1. Ve a Settings.
2. Abre Webhooks.
3. Clic en Add webhook.

![48.png](assets/section-12/48.png)

Rellenamos el formulario:

- `Payload URL`: `http://localhost:8080/github-webhook/`
- `Content type`: `application/json`
- `Which events?` → `Just the push event`.

💡 **Nota:** Después de guardar, GitHub mostrará:

> 🛑 "is not supported because it isn't reachable over the public Internet (localhost)"

Este mensaje es completamente normal en nuestro caso. `GitHub` necesita enviar el `webhook a una URL pública`,
y como `Jenkins` está ejecutándose en `localhost`, `GitHub` no puede alcanzarlo.

Aquí solo estamos documentando el procedimiento y completando los campos tal como deberían configurarse.
En un entorno real, la `Payload URL` debería apuntar a una dirección accesible desde internet para que el
webhook funcione.

### 🛠️ 4. Agregando `githubPush()` al Seed Job `dsl-job`

Ahora configuramos el Seed Job para que los jobs generados reaccionen ante un push.

En el DSL Script del `dsl-job` agregamos:

````groovy
triggers {
    githubPush()
}
````

Esto le indica a `Jenkins` que el job se construirá automáticamente cuando `GitHub` envíe un `webhook`.

![49.png](assets/section-12/49.png)

### ▶️ 5. Construyendo el Seed Job

Ejecutamos el Seed Job (`dsl-job`):

- Si hay advertencias por aprobación de script, las autorizamos en
  📍 `Manage Jenkins` → `In-process Script Approval`
- Una vez ejecutado, verificamos el job generado (`ejemplo-job-DSL`)
- Vemos marcada la opción: `GitHub hook trigger for GITScm polling`
- Esto indica que el `githubPush()` se aplicó correctamente.

![50.png](assets/section-12/50.png)

### ✅ Resultado final

Aunque no podemos hacer una prueba real (porque `GitHub` no puede acceder a `localhost`), dejamos documentada la
configuración completa:

- Jenkins mostrando la Hook URL
- GitHub con el webhook configurado
- DSL Script con githubPush()
- Job generado con el trigger activo

En un entorno con Jenkins accesible desde internet, cada push dispararía automáticamente la construcción.
