...
## 🚀 Parámetros e Inputs Interactivos en Jenkins Pipeline

En `Jenkins Pipeline`, las directivas `parameters` e `input` permiten hacer que un pipeline sea dinámico,
configurable y seguro. Son fundamentales tanto en pipelines simples como en flujos empresariales complejos basados
en CI/CD.

### 📦 Parameters

La directiva `parameters` define parámetros que el usuario debe proporcionar al iniciar el pipeline. Estos parámetros
permiten que una misma automatización sea flexible sin necesidad de modificar el `Jenkinsfile`.

📌 Cómo funcionan internamente:

- Todos los parámetros se exponen como variables de entorno durante la ejecución.
- Se accede a ellos mediante:
    - Groovy: `params.NOMBRE`
    - Shell POSIX (bash, ksh): `${NOMBRE}`
    - PowerShell: `${Env:NOMBRE}`
    - Windows CMD: `%NOMBRE%`
- Jenkins únicamente muestra los parámetros cuando se realiza al menos una ejecución inicial del pipeline.
...
Al guardar y ejecutar por primera vez, Jenkins mostrará el botón: `Build with Parameters`
Desde ahí podrás completar los campos definidos.

![24.png](assets/section-15/24.png)

En el formulario que se nos abre podemos cambiar los valores definidos y proceder a ejecutar.

![25.png](assets/section-15/25.png)

Vemos que en `Console Output` se muestran los valores seleccionados.
...
[Pipeline] {
[Pipeline] stage
[Pipeline] { (Clase de Parametros)
[Pipeline] echo
Hola, como estas Milagros
[Pipeline] echo
FLAG: true
[Pipeline] echo
Eleccion: C
...
#### 📌 Buenas Prácticas (Empresas Reales)

- ✔ Usa parámetros cuando el valor varía entre ejecuciones, no para valores fijos.
- ✔ No uses parámetros para secretos → usa `credenciales`.
- ✔ Documenta cada parámetro con `description`.
- ✔ Para entornos, usa parámetros tipo `choice`: `DEV`, `QA`, `PROD`.
- ✔ Si la ejecución depende de parámetros válidos, valida su contenido al inicio.

## ⏸ Input – Pausar Pipeline para Solicitar Datos

La directiva `input` permite `detener el pipeline` y solicitar aprobación o información adicional.
Jenkins pausará la ejecución hasta que un usuario autorizado responda.

#### 📌 Es ideal para:

- Aprobaciones manuales antes de producción
- Solicitar datos adicionales
- Esperar confirmaciones humanas

### 🧪 Ejemplo Práctico con input

Lo que haremos primero será que a nuestro job `primer-pipeline` le vamos a desmarcar la opción
`Esta ejecución debe parametrizarse` ya que los valores los agregaremos dinámicamente mediante el `input`.

![26.png](assets/section-15/26.png)

Configuramos este pipeline en `primer-pipeline`:
...
Como podemos observar, estamos usando la directiva `input`, así que ahora guardamos los cambios y damos en construir
ahora.

![27.png](assets/section-15/27.png)

Durante la ejecución, `Jenkins` detiene el pipeline. Luego solicita el parámetro:

![28.png](assets/section-15/28.png)

Ingresamos un valor al parámetro anterior (por ejemplo `Milagros`) y damos en el botón `Si, continuar porfavor`. Vemos
que la construcción finaliza correctamente.

![29.png](assets/section-15/29.png)

El log del `Console Output` muestra correctamente la salida.
...
[Pipeline] {
[Pipeline] echo
Hola, Milagros, un placer conocerte.
[Pipeline] }
[Pipeline] // withEnv
[Pipeline] }
[Pipeline] // stage
...
| Directiva            | Uso                                                |
|----------------------|----------------------------------------------------|
| `parameters`         | Permite configurar el pipeline antes de ejecutarlo |
| `input`              | Pausa ejecución y espera interacción humana        |
| `params`             | Objeto para acceder a los parámetros               |
| Variables de entorno | Los parámetros se exportan como env vars           |
| Buenas prácticas     | Seguridad, orden, validación, controles de acceso  |

## 🔧 Triggers & Tools

### 🧰 Tools

La directiva `tools` permite `instalar automáticamente herramientas declaradas en Jenkins` y añadirlas al `PATH` antes
de ejecutar los `stages`. Esto es útil cuando los jobs necesitan versiones concretas de `Maven`, `JDK` o `Gradle` sin
tener que instalarlas manualmente en el agente.

#### 📌 Importante:

- Solo funciona si el pipeline utiliza un `agent` distinto de `none`.
- Las herramientas deben estar previamente configuradas en: `Dashboard` → `Manage Jenkins` → `Tools`
- Cada herramienta recibe un “nombre global” que luego identificamos en el script.
...
1. Jenkins verifica qué herramienta se llamó (ej. apache-maven-3.0.1).
2. Descarga o activa la instalación configurada.
3. Ajusta el PATH del agente para que los comandos mvn, java o gradle sean detectados.
4. El stage ya puede ejecutar comandos de la herramienta seleccionada.

💡 Mejores prácticas de empresas

- Separar herramientas por stage cuando diferentes pasos requieren versiones distintas.
- Declarar tools en el nivel raíz del pipeline para que apliquen a todos los steps.
...
La directiva `triggers` define `cómo y cuándo se vuelve a ejecutar un pipeline automáticamente` sin intervención humana.
Aunque los repositorios modernos usan `webhooks`, los triggers siguen siendo útiles para:

- Tareas periódicas (ETL, backups, limpieza).
- Comprobaciones programadas.
- Monitorización de recursos.
- Pipelines que no dependen de código fuente.

Los triggers disponibles son:

| Trigger    | Descripción                                    |
|------------|------------------------------------------------|
| `cron`     | Ejecuta el pipeline en intervalos definidos    |
| `pollSCM`  | Revisa cambios en el repositorio cada X tiempo |
| `upstream` | Ejecuta este pipeline cuando otro finaliza     |

#### 🕒 Cron

El trigger más usado. Funciona igual que un cron tradicional de Linux.

````groovy
pipeline {
    agent any
    triggers {
        cron('H/2 * * * *') // Se active la ejecución cada 2 minutos
    }
    stages {
        stage('Example') {
            steps {
...
Esto significa que Jenkins:

- Esperará 2 minutos.
- Disparará automáticamente una nueva construcción.
- Repetirá el proceso indefinidamente.

![30.png](assets/section-15/30.png)

Cuando grabas el pipeline y corres una ejecución manual, verás que al cabo de 2 minutos aparece una nueva construcción
marcada como “Automatic”.

![31.png](assets/section-15/31.png)

#### 💡 Nota profesional

Al usar `cron` en `Jenkins`:

- Se recomienda usar el modificador H para distribuir cargas de forma uniforme.
    - Ejemplo: `H */4 * * *` → `Jenkins` elige un minuto hash por cada 4 horas.
- Evita usar un solo minuto fijo para muchos jobs (ej. `0 * * * *`) → puede saturar el nodo.

## 🧩 Post (Mejorado y Expandido)

La sección `post` define `acciones que se ejecutan al finalizar un pipeline o un stage`, dependiendo de dónde se
declare.

Estas acciones suelen utilizarse para:

- Enviar `notificaciones` (Slack, email, Teams).
- `Limpiar` archivos temporales.
- Publicar artifacts.
- Actualizar estados en herramientas externas (Jira, GitLab, Sonar, etc.).
- Ejecutar scripts finales independientemente del estado del pipeline.

El comportamiento depende del `estado final` del pipeline o etapa.

### 🎯 Bloques de condición en post

La directiva post admite los siguientes bloques, evaluados en este orden:

| Condición      | Se ejecuta cuando…                                            |
|----------------|---------------------------------------------------------------|
| `always`       | Siempre, haya fallo o éxito                                   |
| `changed`      | Si el estado actual es distinto al anterior                   |
| `fixed`        | Si la ejecución actual **volvió a SUCCESS** tras fallar antes |
| `regression`   | Si la ejecución actual falla y la anterior no                 |
| `aborted`      | Si el usuario detuvo la ejecución                             |
| `failure`      | Si termina en **FAILURE**                                     |
| `success`      | Si finaliza correctamente                                     |
| `unstable`     | Si termina como **UNSTABLE**                                  |
| `unsuccessful` | Si no terminó en SUCCESS (FAILURE, ABORTED, UNSTABLE, etc.)   |
| `cleanup`      | Último bloque, ideal para limpiar artefactos                  |

💡 Nota importante:
> Estos bloques aplican tanto al pipeline completo como a un solo stage.

### 🧪 Ejemplo práctico: detectar un fallo

Vamos a modificar el job `primer-pipeline` desactivando la opción `“Ejecutar periódicamente”` usada en la sección
anterior y añadir un `post` que capture fallos.
...
pipeline {
    ...
    stages {
        stage('Example') {
            steps {
                sh 'Hola' // Esto hará que falle... nos faltó el 'echo'
            }
        }
    }
    post {
...
![32.png](assets/section-15/32.png)

La ejecución falla, mostrando:

![33.png](assets/section-15/33.png)

Y en el `Console Output` se observa que Jenkins entró al bloque failure:
...
[Pipeline] sh
+ Hola
/var/jenkins_home/workspace/primer-pipeline@tmp/durable-a616a4c1/script.sh.copy: 1: Hola: not found
[Pipeline] }
[Pipeline] // stage
[Pipeline] stage
[Pipeline] { (Declarative: Post Actions)
[Pipeline] echo
Esta ejecución ha fallado
[Pipeline] }
[Pipeline] // stage
[Pipeline] }
[Pipeline] // node
...
Como el comando dentro del step es: `sh 'Hola'`:

- El shell intenta ejecutar el binario `Hola`, que no existe.
- Esto produce un exit code 127, lo que establece el estado del pipeline en FAILURE, activando el post.
