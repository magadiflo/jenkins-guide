# 🚀 Sección 8: Conozcamos más sobre Docker en Jenkins

En esta sección aprenderemos cómo hacer que `Jenkins pueda ejecutar comandos Docker desde dentro de su contenedor`, una
necesidad típica cuando `Jenkins` realiza tareas como:
...
La clave no es instalar Docker completo dentro de Jenkins, sino `usar el Docker Engine del host` (tu máquina o
servidor).

Para lograr esto aplicaremos el enfoque estándar utilizado en empresas:

> ✔️ `DooD` – Docker Outside of Docker
...
2. `Montar el socket del host` (`/var/run/docker.sock`). Esto permite que el `Docker CLI` que está dentro de Jenkins
   controle el `Docker Engine rea`l que corre en el host.

> 💡 Este es el patrón recomendado por `Docker`, `Jenkins` y la mayoría de empresas que implementan `CI/CD`
> con contenedores.

## 🐳 1. Instalando Docker CLI en el contenedor de Jenkins

Para agregar `Docker CLI` a `Jenkins`, construiremos una imagen personalizada basada en la imagen oficial:

- 📌 `jenkins/jenkins:2.535-jdk21`
...
## 🐳 Instalando Docker CLI dentro del contenedor de Jenkins
...
- `FROM jenkins/jenkins:2.535-jdk21`
    - Usamos la imagen oficial de `Jenkins` en su versión actual con soporte para JDK 21.
    - Esta será la base sobre la cual añadiremos las herramientas necesarias.
- `USER root`
    - Cambiamos al usuario `root` para poder instalar paquetes y tener permisos sobre el `socket Docker`.
    - ⚠️ Nota técnica: En `Windows` + `Docker Desktop`, si `Jenkins` corre como usuario `jenkins`, no puede acceder
    ...
    ````
    - Por eso definimos `root` aquí, para evitar los problemas de permisos.
- `RUN apt-get update &&...`
    ...
    - ⚠️ `Nota técnica`: Aquí `no instalamos el daemon de Docker`, solo el `CLI`. El daemon ya corre en el host y se
      accede vía el socket. La práctica correcta es que `Jenkins` use el `Docker Engine` del host.

## 🔧 Modificando el compose.yml (con acceso al Docker Host)

El objetivo de esta configuración es permitir que `Jenkins`, **ejecutándose dentro de un contenedor,
pueda interactuar con el Docker Engine del host**. Esto es fundamental cuando Jenkins necesita:

- Construir imágenes Docker.
- Ejecutar contenedores como parte de un pipeline.
- Desplegar microservicios.
- Ejecutar pruebas que dependen de contenedores (Testcontainers, integración, etc.).

Para lograrlo, combinamos dos piezas clave:

1. `Una imagen personalizada` con Docker CLI (vista en la sección anterior).
2. `El montaje del socket del host` (`/var/run/docker.sock`) para acceder al Docker Engine real.

A continuación, se muestra el archivo `compose.yml` actualizado.

### 🐳 compose.yml con acceso al Docker Host
...
- `restart: unless-stopped`
    - Configuramos el contenedor para reiniciarse automáticamente, excepto si se detiene manualmente.
- Exponemos los puertos necesarios:
    - `8080`: acceso web a Jenkins.
    - `50000`: comunicación con agentes remotos.
- Volumes
    - `jenkins_home` volumen nombrado para persistir la configuración y datos de Jenkins.
    - `/var/run/docker.sock`: bind mount del socket Docker del host.
    - ⚠️ Nota técnica: Este socket es lo que permite que el `Docker CLI` dentro del contenedor se comunique con el
      daemon del host.
...
Ahora ejecutamos nuevamente el docker compose pero para levantar el contenedor de jenkins que ahora usará el Dockerfile
para la creación del contenedor.

````bash
D:\programming\jenkins\jenkins-guide (section-09)                                                                                                                                                                                  
...
Porque cuando cambias el `Dockerfile` o cualquier archivo que forme parte del `build context`, `Docker Compose` podría
reutilizar una imagen cacheada. Con `--build`, fuerzas explícitamente a que se genere una `nueva imagen`,
garantizando que:

- Se instale la versión correcta del `Docker CLI` dentro del contenedor.
- Cualquier cambio en el `Dockerfile` se refleje inmediatamente.

## ✔️ Verificando construcción del nuevo contenedor
...
jenkins               latest                     cba75513c86c   11 hours ago    697MB
jenkins/jenkins       2.535-jdk21                84444d75a07c   13 days ago     491MB
````

Observamos dos imágenes:

- `jenkins/jenkins:2.535-jdk21` → Es la imagen base oficial que utilizamos en nuestro Dockerfile.
- `jenkins:latest` → Es la imagen personalizada, construida por Docker Compose usando nuestro Dockerfile
...
627253fe0ea5   jenkins   "/usr/bin/tini -- /u…"   About a minute ago   Up 56 seconds   0.0.0.0:8080->8080/tcp, [::]:8080->8080/tcp, 0.0.0.0:50000->50000/tcp, [::]:50000->50000/tcp   c-jenkins
````

## 🐳 Ejecutando comandos de Docker desde dentro del contenedor de Jenkins

Ahora que tenemos nuestro nuevo contenedor de `Jenkins` construido y en ejecución, probamos si realmente puede
...
# docker container ls -a
CONTAINER ID   IMAGE     COMMAND                  CREATED              STATUS              PORTS                                                                                          NAMES
627253fe0ea5   jenkins   "/usr/bin/tini -- /u…"   About a minute ago   Up About a minute   0.0.0.0:8080->8080/tcp, [::]:8080->8080/tcp, 0.0.0.0:50000->50000/tcp, [::]:50000->50000/tcp   c-jenkins
# docker image ls
...
1b23fd9adbcd   bridge        bridge    local
e067f6421fbb   host          host      local
aa50555c0963   jenkins-net   bridge    local
...
ejecutándolos dentro del contenedor.

````bash
$ docker container ls -a
CONTAINER ID   IMAGE     COMMAND                  CREATED         STATUS         PORTS                                                                                          NAMES
627253fe0ea5   jenkins   "/usr/bin/tini -- /u…"   3 minutes ago   Up 3 minutes   0.0.0.0:8080->8080/tcp, [::]:8080->8080/tcp, 0.0.0.0:50000->50000/tcp, [::]:50000->50000/tcp   c-jenkins
````

Resultados:

- Se listan los mismos contenedores, imágenes, volúmenes y redes que en el host.
- ✅ Jenkins tiene acceso completo al Docker Host gracias al `socket` y al uso de `USER root`.
- Esto confirma que nuestro entorno `DooD` – `Docker-outside-of-Docker` está funcionando correctamente.

## 📌 Conclusión

- `Problema de permisos`: En `Windows` + `Docker Desktop`, el usuario `jenkins` no puede acceder al socket.
- `Solución aplicada`: Definir `USER root` en el `Dockerfile`.
- `Configuración lograda`:
    - Jenkins con Docker CLI instalado
    - Jenkins usando el Docker Compose plugin
    - Acceso total al Docker Engine del host vía `/var/run/docker.sock`
    - Sin necesidad de instalar Docker Engine dentro del contenedor (⚠️ mala práctica evitada)
- Ventaja del enfoque `DooD`:
    - ✔️ Ligero
    - ✔️ Seguro
    - ✔️ Fácil de mantener
    - ✔️ Usado ampliamente en pipelines reales de CI/CD
